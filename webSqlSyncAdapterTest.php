<?php
// Name   : webSqlSyncAdapterTest.php - called by the webSqlApp (index.html) For test to debug server side
// Goal   : To communicate data from a MySQL query using myJob to a webSqlSync json format. Result: It works server to client until now. All server table data to Client.
// Version: 2013-10-16
...
// To test: use  http://www.affairesup.com/webSqlApp/webSqlSyncAdapterTest.php				//?username=exp&password=4success
...
if ($CLnum_rows){
	include("webSqlSyncHandlerTest.php");
...
function myJob($handler){
	...
	echo "webSqlSyncAdapterTest - clientLastSyncDate: ", $clientLastSyncDate, "<br>";

	//print_r($handler -> clientData); 
	//$clientData = $handler -> clientData;	// It gives a 10 digits unix date time
	//X// print_r($clientData); 

	//X// $clientLastSyncDate = $clientData['info']['lastSyncDate']/1000;	// It gives a 10 digits unix date time
	//$clientLastSyncDate = $handler -> clientData['info']['lastSyncDate']/1000;	// It gives a 10 digits unix date time
	//X// echo "webSqlSyncAdapterTest - clientLastSyncDate: ", $clientLastSyncDate, "<br>";
	require_once('setContactTest.php');	// With the JSON, it does many INSERTs or UPDATEs to MySQL following some conditions
...
function getServerData($clientLastSyncDate){		//get the modified data from the server using an associative array
	echo "clientLastSyncDate: ", $clientLastSyncDate, "<br>";
